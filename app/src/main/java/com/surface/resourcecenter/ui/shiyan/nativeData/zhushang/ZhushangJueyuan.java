...
public class ZhushangJueyuan extends BaseFragment implements View.OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.test_refresh:
                ...
                TestItemsBean bean =getTestItems(ZhuShangShiyanItem.fzhkzhljy);
    ...
    private void saveShiyanData(ArrayList<EditText> results,boolean isChecked){
        ...
        try {
            ...
            for(int i = 0;i<results.size();i++){
                json.put(ZhuShangShiyanItem.fzhkzhljy_data[i],results.get(i).getText().toString());
            }
            if(isChecked){
                json.put("result","合格");
            } else {
...
//                Log.e(TAG,"mDataCallback result "+response.get().toString());
    ...
    private void updateShiyanData(String result){
        ...
        try {
            ...
            for(int i = 0;i<ZhuShangShiyanItem.fzhkzhljy_data.length;i++){
                String data = jsonObject.getString(ZhuShangShiyanItem.fzhkzhljy_data[i]);
