...
public class ZhushangGongpinNaiya extends BaseFragment implements View.OnClickListener {
    ...
    private void initGridLayout1(){
        String[] header = {"测试部位","施加电压(峰值)(kV)","修正后施加电压（kV）","加压时间(s)","测量或观察结果"};
        String[] leftheader = {"ABC-abc","abc-ABC"};
        String[] leftheader1 = {"48","48"};
        String[] leftheader2 = {"60","60"};
        int ColumnNum = header.length;
        int RowNum = leftheader.length+1;
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.test_refresh:
                ...
                TestItemsBean bean =getTestItems(ZhuShangShiyanItem.dlqgpny);
    ...
    private void saveShiyanData(boolean isChecked){
        ...
        try {
            ...
            for(int i = 0;i<results.size();i++){
                json.put(ZhuShangShiyanItem.dlqgpny_data_h[i],results.get(i).getText().toString());
            }
            for(int i = 0;i<results1.size();i++){
                json.put(ZhuShangShiyanItem.dlqgpny_data_f[i],results1.get(i).getText().toString());
            }
            if(isChecked){
                json.put("result","合格");
            } else {
...
//                Log.e(TAG,"mDataCallback result "+response.get().toString());
    ...
    private void updateShiyanData(String result){
        ...
        try {
            ...
            for(int i = 0;i<ZhuShangShiyanItem.dlqgpny_data_h.length;i++){
                String data = jsonObject.getString(ZhuShangShiyanItem.dlqgpny_data_h[i]);
                results.get(i).requestFocus();
                results.get(i).setText(data);
            }
            for(int i = 0;i<ZhuShangShiyanItem.dlqgpny_data_f.length;i++){
                String data = jsonObject.getString(ZhuShangShiyanItem.dlqgpny_data_f[i]);
