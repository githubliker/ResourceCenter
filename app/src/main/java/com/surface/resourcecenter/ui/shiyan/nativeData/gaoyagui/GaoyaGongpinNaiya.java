    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.test_refresh:
                ...
                TestItemsBean bean =getTestItems(GaoyaShiyanItem.gygpny);
                getShiyanData(0,sampleId,bean.getId(),mDataCallback);
                break;
            case R.id.test_save:
                saveShiyanData(mViewList.get(0).getShiYanData(),mViewList.get(1).getShiYanData(),mTestResult.isChecked());
                break;
        }
    }

    private void saveShiyanData(ArrayList<EditText> results,ArrayList<EditText> results1,boolean isChecked){
        DoTaskActivity activity = (DoTaskActivity) getActivity();
        TestItemsBean bean =getTestItems(GaoyaShiyanItem.gygpny);
        ...
        try {
            ...
            for(int i = 0;i<results.size();i++){
                json.put(GaoyaShiyanItem.gygpny_h_data[i],results.get(i).getText().toString());
            }
            for(int i = 0;i<results1.size();i++){
                json.put(GaoyaShiyanItem.gygpny_f_data[i],results1.get(i).getText().toString());
            }
            if(isChecked){
                json.put("result","合格");
            } else {
...
//                Log.e(TAG,"mDataCallback result "+response.get().toString());
    ...
    private void updateShiyanData(String result){
        ...
        try {
            ...
            for(int i = 0;i<GaoyaShiyanItem.gygpny_h_data.length;i++){
                String data = jsonObject.getString(GaoyaShiyanItem.gygpny_h_data[i]);
                results.get(i).requestFocus();
                results.get(i).setText(data);
            }
            for(int i = 0;i<GaoyaShiyanItem.gygpny_f_data.length;i++){
                String data = jsonObject.getString(GaoyaShiyanItem.gygpny_f_data[i]);
