...
public class JPWensheng_Dianrong extends BaseFragment implements View.OnClickListener {

    private List<GridLayoutBean> mViewList = new ArrayList<>();
    private LinearLayout mFatherLayout;
    private String[] gridHeader = {"环境温升","温升试验（电容补偿回路）","单触头测量点"};
    public JPWensheng_Dianrong(){
    ...
    private void initGridLayout1(){
        String[] header = {"探头编号","允许温升值（K）","实测温升（K）","实测温升（K）","实测温升（K）","实测温升（K）"};
        ...
        for(int m = 0 ;m<RowNum;m++){
            for(int i = 0;i<ColumnNum;i++){
                ...
                rowSpec=GridLayout.spec(m, 1, GridLayout.FILL);
                if(m == 0 &&i == 2){
                    columnSpec=GridLayout.spec(i, 4, 0.75f);
                } else if(m == 0 && (i == 3 || i == 4 || i == 5)){
                    continue;
                }else {
                    columnSpec=GridLayout.spec(i, 1, 0.25f);
                }
