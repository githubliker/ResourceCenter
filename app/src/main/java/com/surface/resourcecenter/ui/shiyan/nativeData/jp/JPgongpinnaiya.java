...
public class JPgongpinnaiya extends BaseFragment implements View.OnClickListener {

    private List<GridLayoutBean> mViewList = new ArrayList<>();
    private LinearLayout mFatherLayout;
    private String[] gridHeader = {"工频耐压试验","绝缘电阻验证"};
    public JPgongpinnaiya(){
    ...
    private void initGridLayout1(){
        String[] header = {"检验部位","检验要求","实测值（GΩ）"};
        String[] leftheader = {"A-（B，C，N，PE）","B-（A，C，N，PE）","C-（A，B，N，PE）"};
        String[] leftheader1 = {"每条电路的绝缘电阻按标称电压至少为1000Ω/V，\n则认为试验通过。"};
        int ColumnNum = header.length;
        int RowNum = leftheader.length+1;
