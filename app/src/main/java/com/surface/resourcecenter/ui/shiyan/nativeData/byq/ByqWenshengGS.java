...
public class ByqWenshengGS extends BaseFragment implements View.OnClickListener {

    private List<GridLayoutBean> mViewList = new ArrayList<>();
    private LinearLayout mFatherLayout;
    private String[] gridHeader = {"空载条件下温升试验结果","负载条件下温升试验结果","绕组总温升Δθcˊ=Δθc[1+（Δθe/Δθc）^1.25]^0.8","短时过负载能力试验"};
    public ByqWenshengGS(){
    ...
    private void initGridLayout(){
        String[] header = {"测试绕组","电源断开瞬间的绕组平均温度θe (℃) ","环境温度(℃)θa","绕组平均温升(K)Δθe=θe-θa "};
        String[] leftheader = {"高压绕组","低压绕组"};
        ...
        for(int m = 0 ;m<RowNum;m++){
            for(int i = 0;i<ColumnNum;i++){
                ...
                if(i == 2 && m != 0){
                    if(m == 1){
                        rowSpec=GridLayout.spec(m, 2, GridLayout.FILL);
                    } else {
                        continue;
                    }
                }
    ...
    private void initGridLayout2(){
        String[] header = {"测试绕组","测量值"};
        String[] leftheader = {"高压绕组平均温升（K）","低压绕组平均温升（K）"};
        int ColumnNum = header.length;
        int RowNum = leftheader.length+1;
    ...
    private void initGridLayout3(){
        String[] header = {"试验时间（h）","顶层油温度（℃）","环境温度（℃）","顶层油温升(K)：≤85"};
        String[] leftheader = {"1","2"};
        int ColumnNum = header.length;
        int RowNum = leftheader.length+1;
